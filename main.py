...
class Matrix:
    def __init__(self, app, font_size=5):
        ...
        self.screenshotCol=0
        [os.remove(png) for png in glob.glob("./screenshot/*.png")]
    ...
    def run(self):
        ...
        self.draw()
        if(self.screenshotCol<40):
            pg.image.save(self.app.surface, "screenshot/screen{:02d}.png".format(self.screenshotCol))
            self.screenshotCol+=1
    ...
    def change_chars(self, frames):
        mask = np.argwhere(frames % self.char_interval == 0)
            ...
            for x, char in enumerate(row):
                if char:
                    ...
                    if red and green and blue:
                        ...
                        color=255 if 160 < color< 250 else color
...
class MatrixVision:
    def __init__(self):
        self.RES=self.WIDTH, self.HEIGHT=1300,750
        pg.init()
    ...
    def end(self):
        ...
        frames[0].save('animated.gif', append_images=frames[1:], save_all=True, duration=300, loop=0)
        quit()

    def run(self):
        while True:
            self.draw()
            [self.end() for i in pg.event.get() if i.type == pg.QUIT]
            pg.display.flip()
            self.clock.tick(30)
